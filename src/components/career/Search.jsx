...
const Search = () => {
  ...
  const handleSearch = () => {
    if (searchValue.trim()) {
      // Only redirect if there's search text
      // Redirect to CareerSearch page with search query and filters as parameters
      ...
      console.log(
        `Redirecting to career search with query: ${searchValue.trim()} and filters:`,
        filters
      );
    }
  };
  ...
  return (
    <section
      ref={sectionRef}
      className="w-full bg-[#F6F6F6] py-10 sm:py-10 md:py-12 pb-8 sm:pb-8 md:pb-8 px-4 sm:px-6 lg:px-8"
    >
      <div className="flex flex-col items-center justify-center max-w-4xl mx-auto gap-6 sm:gap-8 md:gap-10">
        ...
        <h2
          ref={headingRef}
          className="text-center text-[#252525] font-jost text-3xl sm:text-4xl md:text-[40px] lg:text-[48px] font-semibold leading-[120%] w-full transition-all duration-500 ease-out"
          style={{ opacity: 0, transform: "translateY(20px)" }}
        >
          <span className="text-[#EC1C26]">Discover</span> Roles That Inspire
          You
        </h2>

        {/* Subheading - Already responsive */}
        <p
          ref={subheadingRef}
          className="text-center text-[#252525] font-jost text-lg sm:text-xl md:text-[21px] lg:text-[24px] font-normal leading-[120%] w-full max-w-[800px] transition-all duration-500 ease-out"
          style={{ opacity: 0, transform: "translateY(20px)" }}
        >
          Search and apply for jobs that match{" "}
          <span className="text-[#EC1C26]">your talent</span>
        </p>

        {/* Search Bar with animation */}
        <div
          ref={searchBarRef}
          className="flex items-center justify-between w-full max-w-[280px] xs:max-w-[350px] sm:max-w-[450px] md:max-w-[500px] lg:max-w-[608px] h-[60px] sm:h-[65px] md:h-[72px] px-4 sm:px-5 md:px-6 py-[10px] bg-white rounded-[50px] shadow-[0px_0px_4px_2px_rgba(0,0,0,0.25)] transition-all duration-500 ease-out"
          style={{ opacity: 0, transform: "translateY(20px)" }}
        >
          ...
          {/* Filter icon */}
          <div
            className="flex-shrink-0 ml-1 cursor-pointer hover:opacity-80 transition-opacity"
          ...
          >
            <svg
              ...
              viewBox="0 0 40 40"
              fill="none"
              className="drop-shadow-md w-[25px] h-[25px] sm:w-[30px] sm:h-[30px] md:w-[40px] md:h-[40px]"
            >
              ...
              <defs>
                ...
                >
                  ...
                  <feBlend
                    ...
                    result="effect1_dropShadow_893_4526"
                  />
                  <feBlend
                    mode="normal"
                    in="SourceGraphic"
                    in2="effect1_dropShadow_893_4526"
                    result="shape"
                  />
                </filter>
              </defs>
        ...
        {(filters.jobType.length > 0 || filters.experienceLevel.length > 0) && (
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-4 max-w-[280px] xs:max-w-[350px] sm:max-w-[450px] md:max-w-[500px] lg:max-w-[608px] mx-auto">
            <span className="text-gray-700 font-jost text-sm md:text-base">
              Filters:
            </span>
              ...
              <div className="grid grid-cols-2 gap-3">
                {["Entry-level", "Mid-level", "Senior", "Executive"].map(
                  (level) => (
                    <div
                      key={level}
                      className={`px-4 py-2 rounded-full border-2 cursor-pointer transition-all shadow-sm ${
                        filters.experienceLevel.includes(level)
                          ? "border-[#EC1C26] bg-red-100 text-[#EC1C26]"
                          : "border-gray-300 hover:border-gray-400 text-gray-700"
                      }`}
                      onClick={() => {
                        setFilters((prev) => {
                          const newExpLevel = prev.experienceLevel.includes(
                            level
                          )
